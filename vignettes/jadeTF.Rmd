...
```{r, eval=FALSE}
v1 <- bs_var_tf(Y=y1, lambda=fit0$lambda[1], sample.size=10)
v2 <- bs_var_tf(Y=y2, lambda=fit0$lambda[2], sample.size=15)
...
path_wts <- jade_path(fit0 = fit0_withwts, n.fits = 100, out.file = "example_path_wts.RData", log.gamma.min=-1)
path_planned_wts <- jade_path_planned(fit0=fit0_withwts, out.file="example_path_planned_wts.RData", log.gamma.start=0.05, log.gamma.stop=2.3, n.fits=100)
```
