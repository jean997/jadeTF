...
```
The function `jade_path` doesn't require any knowledge about what the critical values of `gamma` at which the curves are totally fused at the high end or just begin to fuse at the low end. `jade_path` can be slower due to fitting more than the target number of fits - for example the above code takes 11.3 minutes and results in 108 fits rather than 100. There is an alternate function `jade_path_planned` which requres inputting a starting and approximate stopping point but can be faster if the points provided are good guesses. The code below takes 10.6 minutes and fits JADE 101 times but has the advantage of knowing where to start and stop.
```{r, eval=FALSE}
...
cv <- cv_err_wts(orig.path="example_path.RData", cv.path.list=cv.path.list)
```

```{r, eval=FALSE}
path <- getobj("example_path.RData")
f <- path$JADE_fits[[cv$cv.1se.l1]]
...
Estimate the variance of a smoothed fit to each group by bootstrapping:
```{r, eval=FALSE}
set.seed(20000)
v1 <- bs_var_tf(Y=y1, lambda=fit0$lambda[1])
v2 <- bs_var_tf(Y=y2, lambda=fit0$lambda[2])
fit_var <- cbind(v1$var, v2$var)
```

Follow the same procedure as before -- fit first at `gamma = 0` and then for a grid of gamma values.
```{r, eval=FALSE}
fit0_wts <- jade_admm(y, gamma=0, ord=2, sample.size = c(10, 15), fit.var=fit_var)
path_wts <- jade_path(fit0 = fit0_wts, n.fits = 100, out.file = "example_path_wts.RData")
path_planned_wts <- jade_path_planned(fit0=fit0_wts, out.file="example_path_planned_wts.RData", log.gamma.start=0.05, log.gamma.stop=1.7, n.fits=100)
```

Cross validation for the weighted analysis:
```{r, eval=FALSE}
cv_fit0(orig.fit=fit0_wts, n.folds=5, which.fold=1:5, save.prefix="f0_wts", return.objects=FALSE)
```
```{r, eval=FALSE}
for(i in 1:5){
  path.cv <- jade_path(fit0 = paste0("f0_wts.", i, ".RData"), n.fits = 100, out.file = paste0("example_path_wts.", i, ".RData"))
}
```
```{r, eval=FALSE}
cv.path.list <- paste("example_path_wts.", 1:5, ".RData", sep="")
cv <- cv_err_wts(orig.path="example_path_wts.RData", cv.path.list=cv.path.list)
path <- getobj("example_path_wts.RData")
res.tab <- get_separated_regions(fit = f, new.tol=1e-3)
plot_jade(fits = f$fits, pos = 1:100, maxwidth = 0.5, sep.tab = res.tab$merged)
```
