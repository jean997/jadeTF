...
```{r, eval=FALSE}
fit1 <- jade_admm(y, gamma=1, ord=2, sample.size = c(10, 15), lambda=fit0$lambda)
```
The `jade_admm` function is the workhorse of the `jadeTF` package and executes the ADMM algorithm described in (reference to paper). The parameters `theta0`, `u.alpha0` and `u.beta0` can be used to start the ADMM algorithm off at the solution for a nearby value of `gamma`. This ''warm start'' techinique can reduce the number of iterations to reach convergence and will be used when we run a sequence of fits using the `jade_path_planned` or `jade_path` functions.
...
```{r}
res.tab <- get_separated_regions(fit = fit1)
res.tab$separated
```
...
```
The function `jade_path` doesn't require any knowledge about what the critical values of `gamma` at which the curves are totally fused at the high end or just begin to fuse at the low end. In its current implementation `jade_path` can be slow and fit many more than the desired number of fits - for example the above code results in 222 fits rahter than 100. There is an alternate function `jade_path_planned` which requres inputting a starting and approximate stopping point but can be faster if the points provided are good guesses. The code below only takes 30 minutes and fits 102 fits but has the advantage of knowing where to start and stop.
```{r, eval=FALSE}
